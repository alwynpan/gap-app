...
function AssignGroupModal({ user, subjects, onClose, onAssigned }) {
  ...
  // Suspended memberships are excluded: assignUserToGroup rejects them with 403,
  // so offering the subject here guarantees a failed action.
  const availableSubjects = (user.subjects || (subjects || []).filter((s) => membershipSubjectIds.has(s.id))).filter(
    (s) => s.membership_enabled !== false
  );
  ...
  return (
    <Modal title={`Assign Group — ${user.username}`} onClose={onClose}>
      {error && (
