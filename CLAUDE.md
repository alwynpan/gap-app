...
G.A.P. (Group Assignment Portal) - A role-based access control system for managing student groups and assignments. Monorepo with three packages: backend, frontend, and e2e tests.
...
- Production Dockerfiles (`Dockerfile`) are separate: backend runs `node src/server.js`, frontend builds static assets and serves via nginx
...
### Pre-commit Hooks
Husky runs `lint-staged` on commit, which applies Prettier and ESLint fixes to staged files. ESLint uses `--max-warnings 0` so any warning fails the lint.
